...
parser.add_argument('--rand_forest', action='store_true', default=False, help='True for Random Forese' + 
												' False for Logistic Regression ')
parser.add_argument('--load_model', action='store_true', default=False, help='True to Load saved model')
...
# Remove 'state' from dataframe
# diagnosed_data_old = diagnosed_data.drop(['state'], axis=1, errors='ignore')
# hot_columns = list(diagnosed_data_old.columns.values)
# print('Initial Dataframe Shape : ', diagnosed_data_old.shape)

# # Feature Reduction
# clf = RandomForestClassifier()
# clf = clf.fit(diagnosed_data_old, diagnosed_col)
# model = SelectFromModel(clf, prefit=True)
# diagnosed_data_new = model.transform(diagnosed_data_old)


# # Get Pandas Dataframe from above recieved numpy array
# new_cols = []
# for i in range(diagnosed_data_new.shape[1]):
# 	for col in hot_columns:
# 		a = np.sum(np.abs(diagnosed_data_new[:,i] - diagnosed_data_old[col]))
# 		if a == 0.0:
# 			new_cols.append(col)
# 			break
# diagnosed_data = pd.DataFrame(diagnosed_data_new, columns = new_cols)
# print('New Dataframe Shape : ', diagnosed_data.shape)
# del diagnosed_data_new, diagnosed_data_old


# Initially on Full Dataset
...
# 	print i
# 	for col in hot_columns:
# 		a = np.sum(np.abs(diagnosed_data_new[:,i]-hot_data_old[col]))
# 		if a == 0.0:
# 			new_cols.append(col)
# 			break
# diagnosed_data = pd.DataFrame(diagnosed_data_new, columns = new_cols)
# print('New Dataframe Shape : ', diagnosed_data)
# del diagnosed_data_new, diagnosed_data_old
...
# for k in range(1):
	...
	if not LOAD_MODEL:
		...
		FINAL_RES[cold_col_names_list[k]] = log_reg.load_model()['error']
