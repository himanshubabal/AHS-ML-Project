...
for k in range(len(cold_col_names_list)):
	...
	diagnosed_d = cold_data.copy()
	diagnosed_c = to_pred_col.copy()

	print('-------------------------------------------------------')
	print("current feature to be randomised : ",cold_col_names_list[k])

	if (cold_col_names_list[k] == 'diagnosed_for'):
		diagnosed_d = replace_col_by_random_values(diagnosed_d, [cold_col_names_list[k]])
	...
	model_path = model_save_path + str(pred_col) + '/' + txt + '/' + str(state) + '/' + \
		'random_data_test' + '/' +str(cold_col_names_list[k]) + '/' + 'sklearn' + '/'
