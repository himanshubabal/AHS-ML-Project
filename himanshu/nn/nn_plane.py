...
class FFNN():
	"""docstring for FFNN"""
	def __init__(self, tst, iter, hidden_layers = 2, num_hidded_units = [200,300,200,300,200,300,200,300,200,300,200,300,200,300,200,300,200,300,200,300], activation = activations[0], l_r = 0.001, lr_decay = 0.01, weigth_initialisation = tf.contrib.layers.xavier_initializer()):
...
y_ = FFNN(tst, iter).create_model(X, [100, 302], keep_prob)
y__ = tf.argmax(y_, 1)
...
output_file = open("nn_plane.txt","w")
	...
	with tf.Session() as sess:
		...
		# for epoch in range(1):
			...
			for i in range(num_batches-1):
				...
				sess.run(_optimize, feed_dict = {X : x_batch, y : y_batch, keep_prob : keep_prob_arr[2], tst : False, iter : i})
			...
			labels = sess.run(y_, feed_dict = {X : test_data1, y : test_label, keep_prob : keep_prob_arr[2], tst : True, iter : epoch})
			print np.sum(labels-prev_labels)
		print str(k)+","+str(sess.run(_error, feed_dict = {X : test_data1, y : test_label, keep_prob : keep_prob_arr[2], tst : True, iter : 1}))+"\n"
		output_file.write(str(k)+","+str(sess.run(_error, feed_dict = {X : test_data1, y : test_label, keep_prob : keep_prob_arr[2], tst : True, iter : 1}))+"\n")
	# # break
	# 	# print list(labels)
	# 	# print accuracy
