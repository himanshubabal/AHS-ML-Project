...
class FFNN():
	"""docstring for FFNN"""
	def __init__(self, hidden_layers = 2, num_hidded_units = [200,300,200,300,200,300,200,300,200,300,200,300,200,300,200,300,200,300,200,300], activation = activations[0], l_r = 0.001, lr_decay = 0.01, weigth_initialisation = tf.contrib.layers.xavier_initializer(), dropout_type = 0):
...
def start(model_params):
	...
	y_ = FFNN(model_params["layers"], model_params["hidden_layer"], model_params["activation"], model_params["lr"], dropout_type = model_params["is_dropout_full"]).create_model(X, [100, model_params["input_dim"]], keep_prob)	
	...
	train_data, train_label, test_data, test_label = read_red_data("/home/rishabh/my_stuff/a/data/", model_params["encoded_file_name"], model_params["input_dim"])
	...
	with tf.Session() as sess:
		...
		# for epoch in range(1):
			prev_labels = sess.run(y_, feed_dict = {X : test_data, y : test_label, keep_prob : model_params["dropout"]})
			...
			for i in range(num_batches-1):
				...
				sess.run(_optimize, feed_dict = {X : x_batch, y : y_batch, keep_prob : model_params["dropout"]})

				if i%100 == 0:
					print "the error after ", i, " iterations is : ", sess.run(_error, feed_dict = {X : x_batch, y : y_batch, keep_prob : model_params["dropout"]})
				# break
				# print np.isnan(sess.run(cross_entropy, feed_dict = {X : test_data, y : test_label}))
			# sess.run(_optimize, feed_dict = {X : x_batch, y : y_batch})
			print "training error is ", sess.run(_error, feed_dict = {X : train_data, y : train_label, keep_prob : model_params["dropout"]})
			print "test error is ", sess.run(_error, feed_dict = {X : test_data, y : test_label, keep_prob : model_params["dropout"]})
			labels = sess.run(y_, feed_dict = {X : test_data, y : test_label, keep_prob : model_params["dropout"]})
			print np.sum(labels-prev_labels)
		return sess.run(_error, feed_dict = {X : test_data, y : test_label, keep_prob : model_params["dropout"]})
